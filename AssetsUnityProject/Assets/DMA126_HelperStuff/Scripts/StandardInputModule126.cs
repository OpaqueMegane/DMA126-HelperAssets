...
public class StandaloneInputModule126 : PointerInputModule
{
    ...
    protected StandaloneInputModule126()
    {
    }
    ...
    [SerializeField]
    private KeyCode m_SubmitKeyCode = KeyCode.None;
    ...
    private void LateUpdate()
    {
        ...
        if (es != null)
        {
            ...
            if (es.currentSelectedGameObject == null && lastSelected != null)
            {
                es.SetSelectedGameObject(lastSelected);
                //foreach (var sel in GameObject.FindObjectsOfType<Selectable>())
                //{
                //    if ()
                //}
                //es.currentSelectedGameObject = es.lastSelectedGameObject;
            }
        }
    }
    ...
    {
        bool shouldActivate = !string.IsNullOrEmpty(m_SubmitButton) && input.GetButtonDown(m_SubmitButton);
        shouldActivate |= m_SubmitKeyCode != KeyCode.None && Input.GetKeyDown(m_SubmitKeyCode);
        return shouldActivate;
    }

    bool cancelButtonDown()
    {
        bool shouldActivate = !string.IsNullOrEmpty(m_CancelButton) && input.GetButtonDown(m_CancelButton);
        //shouldActivate |= m_SubmitKeyCode != KeyCode.None && Input.GetKeyDown(m_SubmitKeyCode);
        return shouldActivate;
    }
    ...
    public override bool ShouldActivateModule()
    {
        ...
        shouldActivate |= submitButtonDown();
        shouldActivate |= cancelButtonDown();// input.GetButtonDown(m_CancelButton);
    ...
    protected bool SendSubmitEventToSelectedObject()
    {
        ...
        if (cancelButtonDown())//input.GetButtonDown(m_CancelButton))
